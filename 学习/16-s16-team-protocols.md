...
> 原章目录：`s16_team_protocols/`
> 一句话：**队友之间要有约定** —— request-response 模式驱动协商。
...
### 2.6 统一 inbox 消费：consume_lead_inbox
...
## 3. 关键改动点（对照 `s16_team_protocols/code.py`）
...
| Lead 工具 | 14 | 14（核心工具集加入 request_shutdown, request_plan, review_plan） |
| 队友工具 | 4 | + submit_plan (5) |

## 4. 通用性：一个状态机对应两种协议

教学版的一个 FSM（pending → approved | rejected）对应两种协议，**这个简化完全正确**。CC 的所有协议消息共用同一个 request id 关联机制。
...
**协议层 vs 业务层分离**：协议只关心"请求-响应-状态机"，不关心请求里 payload 的具体内容。shutdown 的 payload 是关机原因，plan_approval 的 payload 是计划文本，但协议逻辑相同。
...
1. 关机握手是否完整（请求 → 确认 → 关机）？
2. `pending_requests` 的状态是否正确转换？
3. `request_id` 是否在请求和响应之间保持一致？
4. 队友 idle 后是否能收到 shutdown_request？
5. plan_approval_request 发出后，队友在收到 response 之前能不能继续工作？

**推荐 prompt**：

- `Spawn alice as a backend dev. Ask her to create a file. Then request her shutdown.`
- `Spawn bob with a refactoring task. Have him submit a plan first. Then review and approve it.`
...
CC 的 shutdown 是三向通信（`teammateMailbox.ts:720-763`、`SendMessageTool.ts:268-430`）：

- Lead 发 `shutdown_request`
- 队友回复 `shutdown_approved`（或 `shutdown_rejected` 附原因）
- 系统发送 `teammate_terminated` 通知所有相关方

关机确认后系统自动清理 pane（tmux/iTerm2）、unassign 任务、从 team config 移除成员（`useInboxPoller.ts:677-800`）。教学版用 `shutdown_response` 统一命名，真实源码拆成 approved/rejected 两种独立消息。

### 6.2 计划审批

真实源码里 plan approval request 由 `ExitPlanModeV2Tool.ts:263-312` 在 plan-mode-required 队友退出 plan mode 时产生。`useInboxPoller.ts:599-661` 当前会自动回写 approval，并把请求交给 Lead 作为上下文（regular message）。

`SendMessageTool.ts:434-518` 仍保留显式 approve/reject response 能力，审批时可同时设置 `permissionMode`（如"批准但以 plan mode 运行"），响应中可包含 `feedback` 字符串供队友修正后重新提交。**不是简单的"Lead 手动 review_plan 工具"流程**。

### 6.3 消息格式

CC 的协议消息是结构化的 JSON（有 Zod schema 验证），教学版用简单的 `type + metadata` 字典。**字段名也不统一**：permission 用 `request_id`（`teammateMailbox.ts:453-462`），shutdown 和 plan approval 用 `requestId`（`teammateMailbox.ts:684-763`）。

### 6.4 执行门控

CC 的队友有完整的 permission gating。**未获批准的高风险操作会被拦截**，不是可选的。教学版只演示了消息流程，没有实现执行拦截。
...
下一章：[s17 · Autonomous Agents](./17-s17-autonomous-agents.md)
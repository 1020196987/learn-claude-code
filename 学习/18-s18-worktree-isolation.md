...
> 原章目录：`s18_worktree_isolation/`
> 一句话：**各干各的目录，互不干扰** —— 任务管目标，worktree 管目录，按 ID 绑定。
...
**不自动 complete task**——任务完成由队友的 `complete_task` 显式触发。
...
## 3. 关键改动点（对照 `s18_worktree_isolation/code.py`）
...
| Lead 工具 | 14 | + create_worktree, remove_worktree, keep_worktree (17) |
| 队友工具 | 8 | 8（bash/read/write 在 worktree cwd 执行） |

## 4. 安全设计：name 校验

教学版用 `validate_worktree_name` 拒绝：

- 路径穿越（`..`、`/`）
- 非法字符（只允许 `[A-Za-z0-9._-]{1,64}`）

CC 的 `worktree.ts:76-84` 校验同样的规则。`worktree.ts:48` 定义 `VALID_WORKTREE_SLUG_SEGMENT`。
...
1. 两个 worktree 的 `git status` 输出是否显示不同的分支？
2. 队友认领带 worktree 的任务后，bash 命令是否在 worktree 目录下执行？
3. `remove_worktree` 对有改动的 worktree 是否拒绝？
4. `.tasks/` 中的任务在绑定后状态是否仍为 `pending`？
...
当前会话的工作目录直接切换到 worktree——不是 prompt 提醒，而是**进程级目录变更**。
...
### 6.2 AgentTool isolation：子 agent 隔离
...
CC 没有 task-worktree 绑定。Worktree 状态通过 `PersistedWorktreeSession`（`worktree.ts:756-768`）管理，字段包括：
...
**没有 taskId**。`saveWorktreeState()`（`sessionStorage.ts:2883-2920`）以 `type: 'worktree-state'` 写入 session transcript。

教学版用 task 的 `worktree` 字段做绑定，是教学简化。**CC 把 worktree 和 task 作为两个独立系统，通过 Agent 理解上下文来关联**。

---

下一章：[s19 · MCP Plugin](./19-s19-mcp-plugin.md)
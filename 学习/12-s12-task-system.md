...
> 原章目录：`s12_task_system/`
> 一句话：**大目标拆成小任务，排好序，持久化** —— 文件持久化的任务图，多 agent 协作的基础。
...
s05 的 TodoWrite 是当前任务的执行清单，保存在会话内存中。这里需要的是**任务系统**：
...
ID 用 `timestamp + random hex` 生成。CC 用顺序 ID + highwatermark 文件防止 ID 重用，是更严谨的设计。

### 2.3 五个工具

| 工具 | 作用 |
|------|------|
| `create_task` | 创建任务（含可选的 `blockedBy`） |
| `list_tasks` | 列出所有任务（一行摘要） |
| `get_task` | 查看完整任务 JSON |
| `claim_task` | 设置 `owner`，pending → in_progress |
| `complete_task` | 设置 `status = completed`，解锁下游 |
...
## 3. 关键改动点（对照 `s12_task_system/code.py`）
...
| 工具 | 3 (s11) | + create_task, list_tasks, get_task, claim_task, complete_task (8) |
| 生命周期 | — | pending → in_progress → completed（无 release 回退） |

**关键设计**：任务系统与错误恢复是独立层。CC 源码中 `utils/tasks.ts` 只管 CRUD，`query.ts` 的 with_retry/RecoveryState 管错误恢复，互不耦合。教学版省略完整错误恢复来聚焦任务系统。
...
1. `.tasks/` 目录下是否生成了 JSON 文件？
2. 完成任务后，被阻塞的任务是否解锁？
3. 跨进程后 `.tasks/` 是否还在？（任务系统的核心价值是持久化）
4. 尝试 claim 一个被阻塞的任务会发生什么？
...
### 5.2 不是 TodoWrite 的升级，是两个独立系统

CC 中 Task System 和 TodoWrite **同时存在**，通过 `isTodoV2Enabled()` 切换（`utils/tasks.ts:133`）——交互式会话默认启用 Task（V2），非交互式/SDK 默认用 TodoWrite。环境变量 `CLAUDE_CODE_ENABLE_TASKS` 可强制启用 Task。**Task 有 TodoWrite 没有的**：
...
`claimTask()`（`utils/tasks.ts:541-612`）用**双重锁**防竞争：
...
下一章：[s13 · Background Tasks](./13-s13-background-tasks.md)
...
### 2.2 spawn_teammate_thread: 启动队友
...
## 3. 关键改动点（对照 `s15_agent_teams/code.py`）
...
| Lead 工具 | 11 | + spawn_teammate, send_message, check_inbox (14) |
| 队友工具 | — | bash, read_file, write_file, send_message (4) |
| 权限 | 本地决策 | 教学版省略（真实 CC 有冒泡机制） |

循环骨架不变。新增的是**线程模型**（Lead 主循环 + 队友 daemon 线程）和**消息总线**（文件系统做收件箱）。
...
1. Lead 如何启动队友？
2. `.mailboxes/` 目录下的 JSONL 文件长什么样？
3. 队友完成后 Lead 的 inbox 有没有注入到 history？
4. 队友是不是真的并行工作（不是 Lead 顺序 spawn）？
...
队友之间**不能嵌套**（`AgentTool.tsx:273` 明确禁止 "teammates spawning other teammates"）。

---

下一章：[s16 · Team Protocols](./16-s16-team-protocols.md)
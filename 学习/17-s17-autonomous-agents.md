...
> 原章目录：`s17_autonomous_agents/`
> 一句话：**自己看板，自己认领** —— 空闲时轮询，有活就干。

## 1. 设计动机

s16 的队友能通信、能握手关机。但每个队友等 Lead 分配任务——如果任务看板上有 10 个未认领任务，Lead 得手动 assign 10 次。**这不能扩展**。

队友应该自己看任务看板，发现没人做的任务就认领，做完再找下一个。Lead 只需要创建任务和启动队友。
...
s16 的队友做完任务就退出（最多 10 轮）。s17 加了 IDLE 阶段，队友在外层循环中反复 WORK → IDLE。
...
### 2.3 scan_unclaimed_tasks: 扫描任务看板
...
### 2.7 consume_lead_inbox: 统一 inbox 消费
...
## 3. 关键改动点（对照 `s17_autonomous_agents/code.py`）
...
1. 队友是否自动认领了未分配的任务？
2. 有 blockedBy 依赖的任务是否在前置完成后被正确认领？
3. 空闲超时后是否自动关机？
4. IDLE 阶段收到 shutdown_request 是否立即响应？
5. `.tasks/` 目录下的任务状态如何变化？
...
### 6.1 CC 的空闲机制：组合路径，不是单一轮询

教学版用一个 `idle_poll()` 统一处理空闲时的 inbox 检查和任务认领。CC 的实际实现是**四个机制的组合**：
...
`claimTask()`（`utils/tasks.ts:541-612`）用 `proper-lockfile` 的任务文件锁，在锁内完成读-检查-改-写。检查项：owner 是否已存在（`L575-576`）、是否已完成（`L580-581`）、blockedBy 中是否有未完成任务（`L585-594`）。

`claimTaskWithBusyCheck()`（`utils/tasks.ts:614-692`）用 task-list 级别锁，**把 busy check 和 claim 做成原子操作，避免 TOCTOU**。
...
下一章：[s18 · Worktree Isolation](./18-s18-worktree-isolation.md)